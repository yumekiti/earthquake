...
const Component: FC<Props> = ({ body }) => {
  return (
    body.areas.map((area: any, index: number) => (
      <details key={index}><summary>津波予報区名：{area.name}</summary>
        津波予報の詳細
        <ul className="list-disc">
          <li>津波予報の種類：{gradeData.find((grade: any) => grade.value === area.grade)?.label}</li>
